...
public class Sbox {
    ...
    public void firstSbox(String message, String key) {
        int[][] state, keyMatrix = new int[4][4];
        while (message != null) {
            if (message.matches("[0-9A-F]+")) //If line is valid (i.e. contains valid hex characters, encrpyt. Otherwise, skip line.
            {
                if (message.length() < 32) {
                    message = String.format("%032x", Integer.parseInt(message, 16));
                }
                state = new int[4][4];
                for (int i = 0; i < 4; i++) //Parses line into a matrix
                {
                    for (int j = 0; j < 4; j++) {
                        state[j][i] = Integer.parseInt(message.substring((8 * i) + (2 * j), (8 * i) + (2 * j + 2)), 16);
    ...
    /**
     * cipherBytes performs addRoundKey and sbox[][].
     * @param msg message byte, key keybyte
     */
    public int cipherBytes(int msg, int key) {

        int xor = msg^key;
        return sbox[xor / 16][xor % 16];
    }
}
